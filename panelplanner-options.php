...
function panel_planner_mass_stage_email($stage){
	...
	$panels = $wpdb->get_results('SELECT *.'.$panellistsTableName.', *.'.$panelTableName.' from '.$panelTableName.' WHERE approvalStage = '.$stage. 'INNER JOIN '.$panelistsTableName.' on id.'.$panelTableName.' = panelistID.'.$panelistsTableName);
	foreach($panels as $panel){
		$email = $panel->email;
